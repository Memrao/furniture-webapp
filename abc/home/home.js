document.addEventListener('DOMContentLoaded', async function() {
    ...
    arrowImg.src = "./images/down-arrow.png"; // Make sure the path is correct
            ...
            if (Array.isArray(data)) {
                ...
                data.forEach((category) => {
                    ...
                    anchor.addEventListener("click", (event) => {
                        ...
                        window.location.href = `./product-listing/product-listing.html?category=${encodeURIComponent(
                        category.slug
                    )}`;
                    });
    ...
    document.addEventListener('click', function(event) {
        const dropdown = document.getElementById("categoryDropdown");
        const isClickInsideDropdown = dropdown.contains(event.target) || event.target.matches('.dropbtn');
        ...
        if (footerContent) {
            const footerHTML = `
              ...
              <div style="text-align: center; color: #fff; padding: 10px; background-color: #222;">
                  <!-- Footer bottom content (optional) -->
              </div>
          `;
